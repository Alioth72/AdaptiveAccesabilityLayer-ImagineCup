    ...
    return (
        <div className="min-h-screen p-4 md:p-8">
            <div className="max-w-5xl mx-auto">
                <button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-primary mb-8 font-medium">
                            ...
                            <div className="relative z-10 space-y-8 pl-4 border-l-4 border-gray-100 ml-4">
                                {skills.map((skill) => (
                                    <div key={skill.id} className="relative">
                                        <div className={`absolute -left-[29px] top-1/2 -translate-y-1/2 w-8 h-8 rounded-full border-4 border-white shadow-sm flex items-center justify-center transition-colors duration-500
                                           ${skill.unlocked ? (skill.level >= 3 ? 'bg-green-500 text-white' : 'bg-indigo-500 text-white') : 'bg-gray-200 text-gray-400'}
                                        `}>
                                        ...
                                        <motion.div
                                            ...
                                            className={`p-6 rounded-2xl border-2 transition-all ${skill.unlocked
                                                    ? (skill.level >= 3 ? 'bg-green-50 border-green-200' : 'bg-indigo-50 border-indigo-200')
                                        ...
                                        >
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                {skill.unlocked && (
                                                    <span className={`text-xs font-bold px-2 py-1 rounded shadow-sm ${skill.level >= 3 ? 'bg-green-100 text-green-700' : 'bg-indigo-100 text-indigo-700'}`}>
