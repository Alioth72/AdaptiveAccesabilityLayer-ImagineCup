...
import { CheckCircle, HelpCircle, ArrowRight, Star, TrendingUp, XCircle } from 'lucide-react';
...
interface Concept {
    id: string | number;
    questions: {
        remedial?: QuestionData;
        standard: QuestionData;
        advanced?: QuestionData;
        [key: string]: QuestionData | undefined;
    };
}
...
const Quiz: React.FC<QuizProps> = ({ concept, onAnswer, level }) => {
    ...
    const handleSubmit = () => {
        ...
        // Slight delay to show feedback before notifying parent
        setTimeout(() => {
            onAnswer(isCorrect);
        }, 2000);
    };
    ...
    const getFeedbackMessage = () => {
        if (selected === questionData.correctAnswer) {
            return level === 'advanced'
                ? `Advanced Mastery Unlocked! ${questionData.feedback}`
    ...
    return (
      ...
      ${
          level === 'advanced'
              ? 'bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-900 text-white border-2 border-yellow-400/30'
              : level === 'remedial'
        ...
        >
            ...
            <h3
                className={`text-2xl font-semibold mb-6 flex items-center gap-3 ${
                    level === 'advanced' ? 'text-white' : 'text-gray-800'
                }`}
            >
                <span
                    className={`p-2 rounded-xl ${
                        level === 'advanced' ? 'bg-white/10' : 'bg-indigo-100 text-primary'
                    }`}
                >
                    <HelpCircle />
                </span>
            ...
            <p
                className={`text-xl mb-8 font-medium leading-relaxed ${
                    level === 'advanced' ? 'text-indigo-100' : 'text-gray-700'
                }`}
            >
                {questionData.text}
            </p>
            ...
            <div className="grid gap-4 mb-8">
                {questionData.options.map((option, idx) => {
                    ...
                    let cardClass =
                        'w-full text-left p-5 rounded-2xl text-lg font-medium transition-all duration-200 border-2 relative overflow-hidden ';
                    ...
                    if (!submitted) {
                        ...
                        if (isCorrectIndex) {
                            cardClass +=
                                'border-green-500 bg-green-500 text-white scale-[1.01] shadow-lg z-10';
                  ...
                  ${
                      isSelected
                          ? submitted && isCorrectIndex
                              ? 'border-white bg-white text-green-600'
                              : submitted && !isCorrectIndex
                              ? 'border-red-500 text-red-500'
                              : 'border-current'
                          : 'border-gray-300/50 text-transparent'
                  }
                `}
                                >
