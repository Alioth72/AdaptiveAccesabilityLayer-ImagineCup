...
const MindMap: React.FC<MindMapProps> = ({ onBack }) => {
    ...
    useEffect(() => {
        ...
        if (saved) {
            setSkills(JSON.parse(saved) as Skill[]);
        }
    }, []);

    // Layout configuration
    const getNodePosition = (index: number, total: number) => {
        const radius = Math.min(220, total * 40);
        const centerX = 500;
        const centerY = 300;
    ...
    return (
        <div className="min-h-screen bg-indigo-950 text-white p-8 relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="relative h-[calc(100vh-140px)] w-full bg-white/5 rounded-[4rem] border border-white/10 backdrop-blur-sm shadow-2xl overflow-hidden cursor-grab active:cursor-grabbing">
                    <svg
                    ...
                    {skills.map((skill, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`absolute w-[140px] h-[140px] rounded-full flex flex-col items-center justify-center p-4 text-center border-4 transition-all duration-500 shadow-lg ${skill.unlocked
                                        ? skill.level >= 3
                                            ? 'bg-green-500 border-green-300 text-white shadow-green-500/20'
